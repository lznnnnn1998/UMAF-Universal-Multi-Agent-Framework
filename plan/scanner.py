"""PlanScannerRole — project understanding agent for plan pipeline."""
...
class PlanScannerRole(AgentRole):
    """Scans project directory to gather planning-relevant context.

    Produces project_context.json with language, framework, coding conventions,
    directory layout, architecture patterns, dependency manifest, and file
    manifest. Gracefully degrades to empty/minimal context for greenfield projects.
    Modeled after FeatureScannerRole but with planning-specific context extraction
    (architecture patterns, module dependency graph, test coverage patterns).
    """

    agent_name: str = "plan_scanner"
    max_steps: int = 10

    def tools_for_backend(self, backend: str) -> list[dict[str, Any]]:
        """Return Read + WebSearch tool specs for both backends."""
        if hasattr(ToolRegistry, "plan_scanner_tools"):
            return ToolRegistry.to_dicts(ToolRegistry.plan_scanner_tools())
        return ToolRegistry.to_dicts([
            ToolRegistry.READ_FILE, ToolRegistry.WEB_SEARCH,
        ])

    def build_task(self, backend: str, working_dir: str = ".",
                   project_dir: str = ".", **context: Any) -> str:
        """Build the project scanning prompt with backend-aware instructions."""
        common = (
            f"You are a project analyst preparing for implementation planning. "
            f"Scan the project directory and produce a detailed "
            f"project_context.json file with planning-relevant context.\n\n"
            ...
            f"   - Docstring format (google, numpy, sphinx, descriptive)\n"
            f"   - Error handling patterns (try/except, return None, raise)\n"
            f"4. Identify architecture patterns:\n"
            f"   - Module dependency graph (which modules import which)\n"
            f"   - Design patterns in use (factory, strategy, observer, etc.)\n"
            f"   - Class hierarchy depth and structure\n"
            f"5. Read 1-2 test files to determine:\n"
            ...
            f"   - Mock library used\n"
            f"6. Write project_context.json with ALL of the above\n\n"
            f"## Output schema\n"
            f"```json\n"
            f'{{\n'
            f'  "project_dir": ".",\n'
            ...
            f'  "test_directories": ["test"],\n'
            f'  "entry_points": ["main.py"],\n'
            f'  "conventions": {{\n'
            f'    "naming": {{}},\n'
            f'    "import_style": "absolute",\n'
            f'    "type_annotations": "high",\n'
            f'    "docstring_format": "descriptive",\n'
            f'    "error_handling": "exceptions_with_fallbacks"\n'
            f'  }},\n'
            f'  "test_patterns": {{\n'
            f'    "framework": "pytest",\n'
            ...
            f'  }},\n'
            f'  "file_manifest": [\n'
            f'    {{"path": "main.py", "role": "entry_point", "description": "..."}}\n'
            f'  ],\n'
            f'  "architecture": {{\n'
            f'    "paradigm": "",\n'
            f'    "design_patterns": [],\n'
            f'    "class_hierarchy_depth": 0\n'
            f'  }},\n'
    ...
    def parse_result(self, result: AgentResult, working_dir: str = ".",
        ...
        """Extract project context from agent messages, disk file, or fallback."""
        scan: dict[str, Any] = {}
    ...
    def _fallback_scanner(project_dir: str = ".", working_dir: str = ".") -> dict[str, Any]:
        ...
        Gracefully degrades to minimal context for empty/greenfield directories.
        """
        find_cmd = (
        ...
        for sf in source_files[:5]:
            ...
            if '"""' in content:
                conventions["docstring_format"] = "descriptive"
            if "try:" in content:
        ...
        # Detect architecture patterns from source files
        design_patterns: list[str] = []
        for sf in source_files[:10]:
            content = safe_read(os.path.join(working_dir, sf))[:5000]
            if not content:
                continue
            if "@abstractmethod" in content or "ABC" in content:
                if "Template Method" not in design_patterns:
                    design_patterns.append("Template Method")
            if "@dataclass" in content:
                if "Data Transfer Object" not in design_patterns:
                    design_patterns.append("Data Transfer Object")
            if "class " in content and "Factory" in content:
                if "Factory" not in design_patterns:
                    design_patterns.append("Factory")

        class_hierarchy_depth = 3 if design_patterns else 1
        ...
        return {
            ...
            "architecture": {
                "paradigm": "unknown" if not design_patterns else "multi-agent",
                "design_patterns": design_patterns,
                "class_hierarchy_depth": class_hierarchy_depth,
            },
